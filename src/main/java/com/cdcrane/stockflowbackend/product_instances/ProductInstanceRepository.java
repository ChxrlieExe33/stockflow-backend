...
public interface ProductInstanceRepository extends JpaRepository<ProductInstance, UUID> {
    ...
    @Query("""
        SELECT NEW com.cdcrane.stockflowbackend.product_instances.dto.ProductInstanceCountDTO(
                ...
                i.colour,
                COUNT(i)
        ) FROM ProductInstance i WHERE i.product.id = ?1 GROUP BY i.width, i.length, i.colour
    """)
    ...
    @Query("""
        SELECT NEW com.cdcrane.stockflowbackend.product_instances.dto.ProductInstanceCountDTO(
                ...
                null,
                COUNT(i)
        ) FROM ProductInstance i WHERE i.product.id = ?1 GROUP BY i.width, i.length
    """)
    ...
    @Query("""
        SELECT NEW com.cdcrane.stockflowbackend.product_instances.dto.ProductInstanceCountDTO(
                ...
                i.colour,
                COUNT(i)
        ) FROM ProductInstance i WHERE i.product.id = ?1 GROUP BY i.colour
    """)
